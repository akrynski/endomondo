    ...
    try:
        ...
        for i, v in enumerate(klucze):
            if v != 'points':
                ...
                if v == 'duration_s':
                    _www.write("Trening trwał <b>{0}godz. {1}min.</b><br>".format(workout_data[i][v] // 3600,
                                                                                  (workout_data[i][v] % 3600) // 60))
                if v == 'comments':
                    comment_dic_list = workout_data[i]
                    comment_dic = list(comment_dic_list['comments'][0])
                    _www.write("Do treningu {0} dodał komentarz:<br>&nbsp&nbsp&nbsp<i>{1}</i><br>".format(
                        comment_dic[0]['author'], comment_dic[2]['text']))
                elif v == 'routes':
                    ...
                    _www.write(
                        "<div style='display:grid; grid-gap:.5rem;grid-template-columns:repeat(auto-fit,minmax(10px, 1fr))'>")
                    for i, v in enumerate(pictures_dic_list):
                        for item in v:
                            if 'picture' in item:
                                _www.write(
                                    "<img src=../{0} style='float:right;width:65%;height:50%;object-fit:scale-down;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);' alt='fotka'>".format(
                                        item['picture'][0][0]['url']))
                    _www.write("</div>")
                elif v == 'tags':
                ...
                plt.savefig(pic)
                _www.write(
                    "<img src='{0}' style='float:center;width:200px;height:150px;object-fit:scale-down;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);' alt='fotka'>".format(
                        str(pic)))
                # print(czasy)
                # print(czas)
